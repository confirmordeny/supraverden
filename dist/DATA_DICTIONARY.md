...
| Basis_for_assessment | Basis_for_assessment | text |  | strongly recommended | The code for the basis for repo's assessment as to whether the organisation is likely to satisfy the FATF definition of International organisation. |  | begins with "B" followed by one or two digits, B00 is not valid. |  |
| Country | Country code | text |  | mandatory | Country code for the organisation. eu is used for European Union entities and zz is used for all other international organisations. | eu, zz | must consist of two lowercase letters |  |
...
| OpenSanctions_id | OpenSanctions_id | OpenSanctions ID |  | mandatory | OpenSanctions ID code from opensanctions.org. If no identifier exists, [No code found] may be used. |  | no spaces, except [No code found] |  |
...
| Wikidata_code | Wikidata code | text |  | optional | Wikidata property code for the organisation. If no identifier exists, [No code found] may be used. |  | starts with 'Q' remaining characters are digits 0-9, except [No code found]. |  |
